...
from wg_logic import PlayerServerInterface
...
class HttpServer:
    def __init__(self):
        ...
        self.game = PlayerServerInterface()
    ...
    def http_get(self,object_address,headers):
        ...
        if path == '/game/start':
            game_data = self.game.start_game()
            json_response = json.dumps(game_data)
        ...
        if path == '/game/submit':
            try:
                ...
                spelling = query_params['spelling'][0]
                game_data = self.game.submit_spelling([player_id, spelling])
                json_response = json.dumps(game_data)
                return self.response(200, 'OK', json_response, {'Content-Type': 'application/json'})
            except (KeyError, IndexError):
                return self.response(400, 'Bad Request', 'player_id and spelling parameters are required.', {})

        files = glob('./*')
        #print(files)
        thedir='./'
        if (object_address == '/'):
            return self.response(200,'OK','This is a Spelling Bee Game Server',dict())
